...
type PixPendente = { id: string; valor: number; status: string; created_at: string; };
...
export default function App() {
  ...
  const handleScanSuccess = async (decodedText: string) => {
    ...
    try {
      ...
      // Validação proativa de padrão (Prefixo)
      if (prefix && selectedBranch) {
         // Limpa os caracteres especiais para comparação
         const sanitizedPrefix = prefix.toLowerCase().replace(/[^a-z0-9]/g, '');
         const sanitizedBranchName = selectedBranch.name.toLowerCase().replace(/[^a-z0-9]/g, '');
         
         const isMatch = sanitizedPrefix === selectedBranch.id.toString() ||
                         sanitizedBranchName.includes(sanitizedPrefix) || 
                         sanitizedPrefix.includes(sanitizedBranchName) ||
                         // Considera combinações como 'logmaxadmin'
                         sanitizedPrefix.includes('logmax') && sanitizedBranchName.includes(sanitizedPrefix.replace('logmax', ''));
                         
         if (!isMatch) {
            throw new Error(`Alerta de Origem: O padrão de QR Code lido ("${prefix}") não corresponde à filial selecionada (${selectedBranch.name}). Por favor, verifique a filial!`);
         }
      }
